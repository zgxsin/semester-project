...
def data_generator(dataset, config, carla_rate, shuffle=True, augment=False, augmentation=None,
    ...
    image_index = -1
    image_index2 = -1
    image_ids = np.copy(dataset[0].image_ids)
    image_ids2 = np.copy(dataset[1].image_ids )
    error_count = 0
    ...
    #
    # if b <= int( carla_rate * batch_size ):
    ...
    while True:
        try:
            ...
            if b < int( carla_rate * batch_size ):
                ...
                # todo: skip images with no classses
            ...
            # res = a if a > b else b
            i = 1 if b>=int(carla_rate * batch_size) else 0
            logging.exception("Error processing image {}".format(
                dataset[i].image_info[image_id]))
            error_count += 1
            if error_count > 5:
                raise
